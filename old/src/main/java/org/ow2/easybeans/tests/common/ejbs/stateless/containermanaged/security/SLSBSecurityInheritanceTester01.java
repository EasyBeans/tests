 ...
 *
 * --------------------------------------------------------------------------
 * $Id: SLSBSecurityInheritanceTester01.java 5369 2010-02-24 14:58:19Z benoitf $
 * --------------------------------------------------------------------------
 */
...
@Stateless
@Remote
public class SLSBSecurityInheritanceTester01 implements ItfSecurityInheritanceTester {

    /**
     * Logger.
     */
    private static Log logger = LogFactory.getLog(SLSBSecurityInheritanceTester01.class);
    ...
    /**
     * Verifies if method that does not allow the role secondaryrole can be
     * called by a bean with the annotation RunAs("secondaryrole").
     */
    public void callMethodMainRole() {
        try {
            bean.dummyMethod2();
            fail("The method has a rolesAllowed annotation that does not "
                    + "include the secondaryrole, so the bean cannot call this method.");
        } catch (EJBAccessException e) {
            logger.debug("The bean threw an expected exception {0}", e);
        }
    }
