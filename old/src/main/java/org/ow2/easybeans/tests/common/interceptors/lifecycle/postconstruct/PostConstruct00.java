 ...
 *
 * --------------------------------------------------------------------------
 * $Id: PostConstruct00.java 5369 2010-02-24 14:58:19Z benoitf $
 * --------------------------------------------------------------------------
 */
 ...
 */
public class PostConstruct00 {

    /**
     * Log helper.
     */
    private Log logger = LogFactory.getLog(PostConstruct00.class);
    ...
    public void interceptor(final InvocationContext ic){
        logger.debug("PostConstruct was invoked.");
        try {
            if (ic.getTarget() instanceof ItfCheckPostConstruct && ic.getMethod() == null){
                ((ItfCheckPostConstruct) ic.getTarget()).setStatus(true);
            }
            ic.proceed();
        } catch (Exception e) {
            logger.debug("Exception: {0}", e.toString());
            throw new IllegalStateException("Exception in " + PostConstruct00.class.getName());
        }
    }
