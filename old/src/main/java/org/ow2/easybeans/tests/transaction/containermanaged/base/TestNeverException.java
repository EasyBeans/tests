 ...
 *
 * --------------------------------------------------------------------------
 * $Id: TestNeverException.java 5369 2010-02-24 14:58:19Z benoitf $
 * --------------------------------------------------------------------------
 */
...
*              SFSBContainerTransactionNeverRuntime must be deployed for
*              testing stateful session bean. And, the bean
*              SLSBContainerTransactionNeverApp,
*              SLSBContainerTransactionNeverRollback and
*              SLSBContainerTransactionNeverRuntime must be deployed for
...
public abstract class TestNeverException extends TestContainerTransactionException{
     ...
     * transaction NEVER does not has a transaction context, so there is not
     * transaction to be managed.
     * @input -
    ...
    /**
     * Verifies if the container did not make a rollback. The transaction
     * attribute never does not have a transaction context.
