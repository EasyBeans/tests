 ...
 *
 * --------------------------------------------------------------------------
 * $Id: SFSBXmlDescriptorTester01.java 5369 2010-02-24 14:58:19Z benoitf $
 * --------------------------------------------------------------------------
 */
...
public class SFSBXmlDescriptorTester01 implements ItfXmlDescriptorTester {

    /**
     * The stateful bean that has the ejb name defined by deployment descriptor.
     */
    @EJB(beanName = "SFSBBasicBeanByXMLLocal")
    private ItfBasicBeanLocal sfsbLocal;

    /**
     * Verifies if the bean that is defined as stateful in the deployment
     * descriptor is actually a stateful. The method must run correctly if the
     * bean is stateful.
