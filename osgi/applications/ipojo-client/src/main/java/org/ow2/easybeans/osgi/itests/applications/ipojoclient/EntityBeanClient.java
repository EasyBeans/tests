 ...
 *
 * --------------------------------------------------------------------------
 * $Id: EntityBeanClient.java 6148 2012-01-25 14:51:49Z benoitf $
 * --------------------------------------------------------------------------
 */
...
public class EntityBeanClient {
   ...
   private void addEmployee() throws IOException {
      File jFile = new File(outputDirectory + File.separator  + "addIPOJOEmployee.txt");
      PrintWriter writer = new PrintWriter(new FileWriter(jFile, false), true);
     try {
      int id = new Random().nextInt();
      this.sessionFacadeRemote.addEmployee(id, "Guillaume_" + id);
      Employee employee = this.sessionFacadeRemote.findEmployee(id);
